...
(module Utils
  ...
  (time time2 time-void take-drop take drop compose compose-n
	...
	number->char char->number dec->bin dec->bins bin+ bin++ bin+s
	bin->dec repeat flatten depth rdp-sort union-pairs mx-copy)
  ...
  (define (repeat f n) ; repeat f n times
    (if (zero? n)
	f
	(compose (repeat f (sub1 n)) f)))
  ...
  (cond
   ...
   ((member to lst) -1)
   ((pair? (car lst))
    (max (add1 (depth (car lst) to)) (depth (cdr lst) to)))
   (else (depth (cdr lst) to))))
...
;;; ==========================================================
  ...
  ;; copy matrix
  (define (mx-copy mx)
   (let* ((rows (matrix-rows mx))
          (columns (matrix-columns mx))
          (mxc (make-matrix rows columns)))
    (do ([j 0 (add1 j)])
	((= j rows) mxc)
      (do ([i 0 (add1 i)])
	  ((= i columns))
	(matrix-set! mxc i j (matrix-ref mx i j))))))
	   ...
	   (mergep
	    (lambda (l1 l2)
	      (cond
	       ...
	       ((null? l2) l1)
	       ((pair<? (car l1) (car l2))
		(cons (car l1) (mergep (cdr l1) l2)))
	       ((pair>? (car l1) (car l2))
		(cons (car l2) (mergep l1 (cdr l2))))
	       (else (set! intersect (cons (car l1) intersect))
		     (cons (car l1) (mergep (cdr l1) (cdr l2)))))))
	   (mergeps
	    (lambda (l1 l2)
	      (cond
	       ((null? l1) l2)
  ...
  ;;; rdp-sort - remove duplicates (works only for pairs '(1 . 2)
  (define (rdp-sort lst)
    (letrec*
	((pair<?
	  (lambda (p1 p2)
	 ...
	 (merge
	  (lambda (l1 l2)
	    (cond
	     ...
	     (else (cons (car l1) (merge (cdr l1) (cdr l2)))))))
	 (sort
	  (lambda (lst)
	    (cond
	     ((null? lst) '())
  ...
  (define (union-pairs l1 l2)
    (rdp-sort (append l1 l2)))
