...
let is_in gs (x,y) = x >= 0 && x <= gs && y >= 0 && y <= gs

let part1 gs grid antsh antl =
  ...
  let rec antinodes nodes = function
      [] -> nodes
    | c::t ->
      antinodes
        (calc (combinations (Hashtbl.find_all antsh c) ~size:2) @ nodes) t
  in
  List.length (List.sort_uniq compare
                 (List.filter (is_in gs) (antinodes [] antl)))

let find_all_ant gs grid antsh =
  for i = 0 to gs do
    for j = 0 to gs do
      let c = grid.(i).(j) in
...
let part2 gs grid antsh antl =
  ...
  let rec antinodes nodes = function
    ...
    | c::t ->
      antinodes
        (calc2 (combinations (Hashtbl.find_all antsh c) ~size:2) @ nodes) t
  in
    List.length (List.sort_uniq compare
                 (List.filter (is_in gs) (antinodes [] antl)))
...
let day8 file =
  ...
  find_all_ant (gs-1) grid antsh;
  let antl = List.sort_uniq compare
      (List.of_seq (Hashtbl.to_seq_keys antsh)) in
  (part1 (gs-1) grid antsh antl, part2 (gs-1) grid antsh antl)
